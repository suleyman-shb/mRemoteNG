        ...
        private void TvConnections_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    var selectedObjects = ConnectionTree.SelectedObjects.OfType<ConnectionInfo>().ToList();
                    if (selectedObjects.Count == 0) return;

                    if (selectedObjects.Count == 1)
                    {
                        Runtime.ConnectionInitiator.OpenConnection(selectedObjects[0]);
                    }
                    else
                    {
                        var connectableNodes = selectedObjects
